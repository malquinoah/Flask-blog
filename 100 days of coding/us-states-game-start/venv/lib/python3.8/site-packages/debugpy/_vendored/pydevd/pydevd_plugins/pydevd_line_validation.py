...
class LineBreakpointWithLazyValidation(LineBreakpoint):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # we'd need to re-verify it (for instance, the templates could have changed on disk).
...
class ValidationInfo(object):
    ...
    def _collect_valid_lines_in_template(self, template):
        # We cache the lines in the templates itself. Note that among requests the
        # templates may be a different instance (because the templates contents could be
    ...
    def verify_breakpoints_from_template_cached_lines(self, py_db, canonical_normalized_filename, template_breakpoints_for_file):
        '''
        This is used when the lines are already available (if just the templates is available,
