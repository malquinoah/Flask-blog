...
def after_breakpoints_consolidated(plugin, py_db, canonical_normalized_filename, id_to_pybreakpoint, file_to_line_to_breakpoints):
    if IS_DJANGO19_OR_HIGHER:
        ...
        # In general we validate the breakpoints only when the templates is loaded, but if the templates
...
def _is_django_render_call(frame, debug=False):
    try:
        ...
        if IS_DJANGO19:
            # in Django 1.9 we need to save the flag that there is included templates
        ...
        if hasattr(node, 'source'):
            ...
            if IS_DJANGO18:
                ...
                pydev_log.error_once("WARNING: Template path is not available. Please set TEMPLATE_DEBUG=True in your settings.py to make "
                                     "django templates breakpoints working")
            return None
...
def _get_template_original_file_name_from_frame(frame):
    try:
        if IS_DJANGO19:
            ...
            if 'context' in frame.f_locals:
                ...
                if hasattr(context, '_has_included_template'):
                    # if there was included templates we need to inspect the previous frames and find its name
                    back = frame.f_back
                    ...
                    if hasattr(context, 'templates') and hasattr(context.template, 'origin') and \
                            hasattr(context.template.origin, 'name'):
                        return _convert_to_str(context.template.origin.name)
            return None
        elif IS_DJANGO19_OR_HIGHER:
            # For Django 1.10 and later there is much simpler way to get templates name
        ...
        if DebugInfoHolder.DEBUG_TRACE_LEVEL >= 2:
            pydev_log.exception('Error getting django templates filename.')
        return None
...
def can_skip(plugin, main_debugger, frame):
    ...
    if main_debugger.django_exception_break:
        ...
        if module_name == 'django.templates.base':
            # Exceptions raised at django.templates.base must be checked.
            return False
...
def get_breakpoint(plugin, py_db, pydb_frame, frame, event, args):
    ...
    if event == 'call' and info.pydev_state != STATE_SUSPEND and py_db.django_breakpoints and _is_django_render_call(frame):
        original_filename = _get_template_original_file_name_from_frame(frame)
        pydev_log.debug("Django is rendering a templates: %s", original_filename)
        ...
        if django_breakpoints_for_file:
            ...
            # At this point, let's validate whether templates lines are correct.
            if IS_DJANGO19_OR_HIGHER:
            ...
            pydev_log.debug("Tracing templates line: %s", template_line)
